...
list.pkg <- c("plyr", "tidyr", "rlang", "tidyselect", "gWidgets","RGtk2","gWidgetsRGtk2","data.table","RCurl","zoo","RedcapData",
           "devtools", "ReporteRs","stringr", "dplyr","grid","ggplot2","zoo", "lubridate", "digest", "janitor", "gt", "ts", "magrittr", "rsample", "h2o", "tidymodels", "skimr","tibble", "gridExtra", "ggforce", "colorspace", "colorblindr","cowplot","dviz.supp")
...
meta_data = meta_data %>%
  select(-grep("x", colnames(.), ignore.case = T)) %>%
  janitor::clean_names() %>%
...
data = data %>%
  mutate(., bmi_cat = ifelse(bmi < quantile(bmi, probs = c(0.05)), "Underweight",
...
data = data %>%
  mutate(., age_cat = ifelse(age <= 5, "2-5", "6-10"),
         asthma_diag = ifelse(child_asthma == 1 | wheezing == 1| medication_a ==1 | medication ==1 | medication_d ==1 | medication_e ==1| medication_f == 1, 1,0),
...
out_all = data %>%
  ...
  cbind(
    ...
    `\tUnderweigh` = paste0(sum(data$bmi_cat == 'Underweight'), '(', round(sum(data$bmi_cat == 'Underweight')/nrow(data)*100), ')'),
...
chi_sex =  chisq.test( xtabs(value ~ key + sex, data = sexx))
...
age_w = wilcox.test(data$age, data_1$age)
...
bmi_w= wilcox.test(data$bmi, data_1$bmi)
...
f_bmi_cat = fisher.test(xtabs(value ~ key + bmi_cat, data = bmi_cat))
...
f_birth_weight = fisher.test(xtabs(value ~ key + `birth weight`, data = birth_weight))
...
f_hospital = fisher.test(xtabs(value ~ key + hospital, data = hospital))


# height

height_t = t.test(data$height, data_1$height)
height_w = wilcox.test(data$height, data_1$height)
...
p_val_baseline = cbind(
  `Gender, n(%)` = round(chi_sex$p.value,3),
  `Age` = round(age_w$p.value,3),
  `Asthma, n(%)` = round(chi_asthma$p.value,3),
...
addTitle(., value = 'TABLE 1. Baseline characteristics of all participants with and without SDB') %>%
...
```{r logit, fig.cap= "Estomates of logistic regression of PSQ score before operation"}
...
# data = data %>%
#   select(-c(shake_breath_b, apnes_b))
# 
# data_reg = data %>%
#   select(-c(sub_id, sub_id_b))
...
#   bind_cols(test_baked %>% select(psq_binary))
