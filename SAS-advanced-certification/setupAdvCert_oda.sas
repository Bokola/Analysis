...
*************************************************************/
%let path=C:/users/&sysuserid/Analysis/SAS-advanced-certification/cert;
%put &path;
...
%macro unpack(unzip /* Full path pointing to where to create the data */
...
%let url=%str(http://dl.liveweb.sas.com/eldata/advCert94_oda.zip);
...
%unpack(%superq(path),advCert.zip,advCert)
