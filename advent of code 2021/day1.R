...
data <- read_table(file.path(home, "day1_data.txt"))  %>% `colnames<-`("x")

data <- data %>%
  mutate(m = case_when(lag(.) < . ~ "increased",
                       lag(.) > . ~ "decreased",
                       TRUE ~ "NA")) 
data %>% group_by(m) %>% tally()

# or 

sum(data$x > lag(data$x) , na.rm = T)

# part 2 

sum(data$x > lag(data$x, n = 3), na.rm = T)
