...
injuries %>%
  ...
  vroom::vroom_write(file.path(home, "data","injuries.tsv.gz"))

products %>%
  semi_join(top_prod, by = c("code" = "prod1")) %>%
  rename(prod_code = code) %>%
  vroom::vroom_write(file.path(home, "data","products.tsv"))

population %>%
  filter(year == 2017) %>%
  select(-year) %>%
  rename(population = n) %>%
  vroom::vroom_write(file.path(home, "data","population.tsv"))