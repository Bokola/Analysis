...
coef(ridge, s ="lambda.min") %>%
  ...
  ggplot(aes(value, reorder(row, value), color = value > 0)) + geom_point(show.legend = F) + ggtitle("Top 100 influential variables (ridge penalty)") + 
xlab("coefficient") +
ylab(NULL)
```
## **Predicting**
...
```
We do not need to to one-ot encode or standardize our variables as `h2o` will do it for us. However, we need to normalize our response variable if it's skewed and then convert our training and test data into `h2o` objects.
```{r}
